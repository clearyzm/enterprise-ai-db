...
| 文件 | 状态 | 说明 |
|---|---|---|
| `migrations/versions/0001_init.py` | ✅ | 完整 DDL：18 张表 + 5 个扩展 + 所有索引（330 行）|
| `migrations/versions/0002_rls.py` | ✅ | 16 张表启用 RLS + tenant_isolation 策略（97 行）|
| `migrations/versions/0003_seed_permissions.py` | ✅ | 19 条全局 permissions（75 行）|
| `app/db/base.py` | ✅ | SQLAlchemy DeclarativeBase（19 行）|
| `app/db/session.py` | ✅ | async engine + sessionmaker + get_db()（49 行）|
| `app/db/rls.py` | ✅ | set_tenant_context() + set_user_context()（44 行）|
| `app/scripts/seed_demo.py` | ✅ | demo tenant + 3 用户 + 5 角色 + 2 部门 + 1 dataset（256 行）|
| `migrations/env.py` | ✅ | 移除 try/import，正式导入 Base（3 行修改）|
...
# 然后用 psql 验证 RLS（详见项目根目录 PHASE_1_REPORT.md 完整版）
```
