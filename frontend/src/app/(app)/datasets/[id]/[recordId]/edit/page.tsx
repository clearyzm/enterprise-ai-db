 ...
 * - ❌ Does NOT show diff preview before submit
 * - ❌ Does NOT implement optimistic UI update
 * - ✅ Only implements: basic form + optimistic lock + submit to approval
 * 
 * TODO (Phase 10+):
 * - Add Zod validation with zodResolver
 * - Add form auto-save to localStorage
 * - Add before/after diff preview
 * - Add optimistic UI update (show pending state)
 */
...
export default function EditRecordPage({ params }: { params: Promise<{ id: string; recordId: string }> }) {
  ...
  const updateMutation = useMutation({
    ...
    onError: (error: unknown) => {
      ...
      if (error && typeof error === 'object' && 'code' in error && error.code === 'http_409') {
        alert('记录已被其他用户更新，请刷新页面后重试');
      }
    },
  });
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="px-8 py-6">
          ...
          <nav className="flex mb-4 text-sm">
            ...
            <span className="mx-2 text-gray-400">/</span>
            <span className="text-gray-900 font-medium">编辑</span>
          </nav>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                当前版本: v{record.version}，修改后将提交审批
              </p>
            </div>
          </div>
        </div>
